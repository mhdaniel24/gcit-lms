...
public class Menu {
	...
	public static String whichBranchDoYouManage()
	{
		return "1) Enter Branch you manage\n2) Quite to previous";
	}
	
	public static String lib2Menu(ArrayList<Branch> branch)
	{
		...
		{
			menu = menu + Integer.toString(i+1) + ") " + branch.get(i).getBranchName() +","+ branch.get(i).getBranchAddress() + "\n";  
		}
	...
	public static String borr1Option1CheckOutBookMenu(ArrayList<Branch> branches){
		String menu = "Pick the Branch you want to check out from:";
		menu = attachToTheEnd(menu, branches);
		return menu;	
	}
	public static String borr1Option2CheckOutBookMenu(ArrayList<Branch> branches){
		String menu = "Pick the Branch you want to return a book to:";
		menu = attachToTheEnd(menu, branches);
		return menu;	
	}
	private static String attachToTheEnd(String menu, ArrayList<Branch> branches)
	{
		int i;
		for(i = 0; i < branches.size(); i++){
			menu = menu + "\n"+Integer.toString(i+1)+") " + branches.get(i).getBranchName();
		}
	...
	public static String allLibraryBranchesMenu(String beginingClause, ArrayList<Branch> libraryBranches)
	{
		String menu = beginingClause;
		int i;
		for(i = 0; i < libraryBranches.size(); i++){
			menu = menu + "\n"+Integer.toString(i+1)+") " + libraryBranches.get(i).getBranchId()+", "+libraryBranches.get(i).getBranchName()+", " +libraryBranches.get(i).getBranchAddress();
		}
		return addQuitPart(menu, i);
	}
